...
def extract_room_dimension_pairs_from_text(raw_text: str) -> list[dict[str, Any]]:
    ...
    for match in pattern.finditer(text):
        ...
        area_sq_m = round(width * height, 2)
...
def extract_area_statement(raw_text: str) -> dict[str, Any]:
    ...
    patterns = {
        "plot_area_sq_ft": r"PLOT AREA.*?([0-9]+(?:\.[0-9]+)?)\s*(?:SQ\s*FT|SQFT|SF|SFT)",
        "plot_area_sq_m": r"PLOT AREA.*?([0-9]+(?:\.[0-9]+)?)\s*SQ\s*\.?\s*M",
        "ground_floor_area_sq_ft": r"GROUND FLOOR AREA.*?([0-9]+(?:\.[0-9]+)?)\s*(?:SQ\s*FT|SQFT|SF|SFT)",
        "ground_floor_area_sq_m": r"GROUND FLOOR AREA.*?([0-9]+(?:\.[0-9]+)?)\s*SQ\s*\.?\s*M",
        "first_floor_area_sq_ft": r"FIRST FLOOR AREA.*?([0-9]+(?:\.[0-9]+)?)\s*(?:SQ\s*FT|SQFT|SF|SFT)",
        "first_floor_area_sq_m": r"FIRST FLOOR AREA.*?([0-9]+(?:\.[0-9]+)?)\s*SQ\s*\.?\s*M",
        "total_area_sq_ft": r"(?:NET TOTAL|TOTAL AREA|TOTAL BUILT UP AREA).*?([0-9]+(?:\.[0-9]+)?)\s*(?:SQ\s*FT|SQFT|SF|SFT)",
        "total_area_sq_m": r"(?:NET TOTAL|TOTAL AREA|TOTAL BUILT UP AREA).*?([0-9]+(?:\.[0-9]+)?)\s*SQ\s*\.?\s*M",
    }
...
def infer_total_area_sq_ft(room_data: list[dict[str, Any]], area_statement: dict[str, Any]) -> float:
    ...
    if room_data:
        return round(sum(float(r["area"]) for r in room_data), 2)
        ...
        for entity in msp:
            ...
            if dtype in {"LWPOLYLINE", "POLYLINE"}:
                ...
                if is_closed:
                    ...
                    if area > 0:
                        poly_areas.append(round(area, 2))
