...
export default function EnterpriseAnalytics() {
  ...
  const metrics = useMemo(() => {
    ...
    const successRate = files.length
      ? (analyzedCount / files.length) * 100
      : 0
    ...
    const totalEstimatedCost = analyzedFiles.reduce((sum, file) => {
      ...
      return (
        sum +
        file.analysis_result.boq.reduce(
          (boqSum, item) => boqSum + (item.amount || 0),
          0
        )
      )
    }, 0)
    ...
    const avgAnalysisTime = analysisTimes.length
      ? Math.round(
          analysisTimes.reduce((a, b) => a + b, 0) /
    ...
    analyzedFiles.forEach((file) => {
      ...
      file.analysis_result?.boq?.forEach((item) => {
        ...
        costByMaterial.set(
          materialName,
          (costByMaterial.get(materialName) || 0) +
            amount
        )
      })
      ...
      costByProject.set(
        projectName,
        (costByProject.get(projectName) || 0) +
          fileCost
      )
    })
  ...
  const exportCsv = () => {
    const rows = [
      ...
      [
        'Blueprints Analyzed',
        String(metrics.analyzedCount)
      ],
      [
        'Total Estimated Cost',
        String(metrics.totalEstimatedCost)
      ],
      [
        'Avg Analysis Time',
        String(metrics.avgAnalysisTime)
      ],
      [
        'Success Rate',
    ...
    link.href = url
    link.download = `analytics-overview-${new Date()
      .toISOString()
      .split('T')[0]}.csv`
    ...
    return (
      <div className="relative min-h-screen overflow-hidden bg-paper">
        ...
        <div className="relative z-10 min-h-screen flex items-center justify-center">
          <div className="rounded-2xl border border-ink/10 bg-paper/50 backdrop-blur-xl px-6 py-4">
  ...
  return (
    <div className="relative min-h-screen overflow-hidden bg-paper">
      ...
      <div className="relative z-10">
        ...
        <main className="px-6 py-6 lg:px-8 space-y-4">
          ...
          <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
            <div>
              ...
              <p className="text-ink/60 mt-1">
                Track insights and performance of your projects.
              </p>
            </div>

            <div className="flex items-center gap-3">
              <button className="inline-flex items-center gap-2 rounded-xl border border-ink/10 bg-paper/50 backdrop-blur-lg px-4 py-2 text-sm text-ink/90 hover:bg-paper/70 transition">
              ...
              <button
                onClick={exportCsv}
                className="inline-flex items-center gap-2 rounded-xl border border-ink/10 bg-paper/50 backdrop-blur-lg px-4 py-2 text-sm text-ink/90 hover:bg-paper/70 transition"
              >
          ...
          <div className="grid grid-cols-1 gap-3 md:grid-cols-2 xl:grid-cols-5">
            {[
              ...
              },
              {
                ...
                value: `${metrics.avgAnalysisTime} sec`,
                icon: Timer,
              ...
              <div
                key={card.label}
                className="rounded-2xl border border-ink/10 bg-paper/50 backdrop-blur-xl p-4 shadow-lg shadow-black/5"
              >
          ...
          <div className="grid grid-cols-1 gap-3 xl:grid-cols-12">
            ...
            <div className="rounded-2xl border border-ink/10 bg-paper/50 backdrop-blur-xl p-4 xl:col-span-5 shadow-lg shadow-black/5">
            ...
            <div className="rounded-2xl border border-ink/10 bg-paper/50 backdrop-blur-xl p-4 xl:col-span-4 shadow-lg shadow-black/5">
              <h2 className="text-xl font-medium text-ink mb-4">
            ...
            <div className="rounded-2xl border border-ink/10 bg-paper/50 backdrop-blur-xl p-4 xl:col-span-3 shadow-lg shadow-black/5">
              <h2 className="text-xl font-medium text-ink mb-4">
                Projects by Status
              </h2>
              ...
              <div className="space-y-3">
                {metrics.topProjects.length === 0 ? (
                  <p className="text-sm text-ink/60">
                    No project cost data yet.
                  </p>
                ) : (
                  metrics.topProjects.map(
                    (project) => (
                      <div
                        key={project.name}
                        className="rounded-xl border border-ink/10 bg-paper/30 backdrop-blur-lg px-3 py-2"
                      >
              ...
              <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
                <div className="h-56">
                  ...
                  >
                    <PieChart>
                      <Pie
                        data={metrics.materials}
                        dataKey="amount"
                        innerRadius={45}
                        outerRadius={80}
                      >
                        {metrics.materials.map(
                          (entry) => (
                            <Cell
                              key={entry.name}
                              fill={entry.color}
